...
public class DLUtil {
    ...
    private static String getProxyViewActionByActivityType(int activityType) {
        ...
        switch (activityType) {
            case DLConstant.ACTIVITY_TYPE_NORMAL: {
                proxyViewAction = DLConstant.PROXY_ACTIVITY_DEFAULT_ACTION;
                break;
            }
            case DLConstant.ACTIVITY_TYPE_FRAGMENT: {
                proxyViewAction = DLConstant.PROXY_FRAGMENT_ACTIVITY_DEFAULT_ACTION;
                break;
            }
            case DLConstant.ACTIVITY_TYPE_ACTIONBAR:
            case DLConstant.ACTIVITY_TYPE_UNKNOWN:
            default:
                break;
        }
    ...
    private static int getActivityType(String className, ClassLoader classLoader) {
        int activityType = DLConstant.ACTIVITY_TYPE_UNKNOWN;
    ...
    private static int getActivityType(Class<?> cls) {
        ...
        try {
            if (cls.asSubclass(DLPluginActivity.class) != null) {
                activityType = DLConstant.ACTIVITY_TYPE_NORMAL;
                return activityType;
            }
        ...
        try {
            if (cls.asSubclass(DLPluginFragmentActivity.class) != null) {
                activityType = DLConstant.ACTIVITY_TYPE_FRAGMENT;
                return activityType;
            }
        } catch (ClassCastException e) {
            // ignored
    ...
    public static void showDialog(Activity activity, String title, String message) {
        new AlertDialog.Builder(activity).setTitle(title).setMessage(message).setPositiveButton("确定", null).show();
    }
}
